...
#https://github.com/tl2cents/AEAD-Nonce-Reuse-Attacks/tree/main
#thank you so much to the person who made this ^
from pwn import *
...
with remote("activist-birds.picoctf.net", 62872) as p:
	#get the messages
	pts = []
	cts = []
	tags = []
	...
	for i in range(2):
		...
		ciphertext = bytes.fromhex(p.recvline().decode("ascii"))

		#add all the data
		pts.append(plaintext)
		cts.append(ciphertext[:-28])
		tags.append(ciphertext[-28:-12])
		nonce = ciphertext[-12:]

	#run the forgery attack
	forged_ct, forged_tag = chachapoly1305_forgery_attack_general(
		[b"",b""], #no headers/associated data
		cts, #ciphertexts
		tags, #MAC tags
		pts[0], #plaintext 1 to get the keystream
		b"But it's only secure if used correctly!", #target plaintext
		b"" #no associated data to forge
	)

	#construct our forged message
	forged = forged_ct + forged_tag + nonce
	print(f"Forged message: {forged}")

	#send it off
	p.sendlineafter(b"message? ", forged.hex().encode("ascii"))
	p.interactive()